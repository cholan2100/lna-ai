...
scale_x = 25.11
scale_y = 25.093
y_rf = 587.8  # Y = 15.0 mm in KiCad

# Ports coordinates in pixels
p1_x = x0 + 0.0 * scale_x       # X = -23.0 mm openEMS (KiCad 0.0)
p2_x = x0 + 8.5 * scale_x       # X = -14.5 mm openEMS (KiCad 8.5)
p3_x = x0 + 25.5 * scale_x      # X = +2.5 mm openEMS (KiCad 25.5)
p4_x = x0 + 46.0 * scale_x      # X = +23.0 mm openEMS (KiCad 46.0)

# Draw Input Trace highlight (Port 1 to Port 2)
trace_in = patches.Rectangle((p1_x, y_rf - 22), p2_x - p1_x, 44, 
...
trace_out = patches.Rectangle((p3_x, y_rf - 22), p4_x - p3_x, 44, 
                              linewidth=2.5, edgecolor='#00f0ff', facecolor='#00f0ff', alpha=0.35, zorder=3)
ax.add_patch(trace_out)

# Draw Central Ground / Isolation Zone (Port 2 to Port 3)
iso_zone = patches.Rectangle((p2_x, y_rf - 130), p3_x - p2_x, 260, 
...
ports = [
    (1, p1_x, y_rf, 'Port 1: SMA RF In', '-23.0 mm', '0.0 mm', '#ff2222', (p1_x - 140, y_rf - 280)),
    (2, p2_x, y_rf, 'Port 2: In Match Pad', '-14.5 mm', '8.5 mm', '#ff8800', (p2_x - 40, y_rf - 340)),
    (3, p3_x, y_rf, 'Port 3: Out Match Pad', '+2.5 mm', '25.5 mm', '#00e676', (p3_x - 40, y_rf - 340)),
    (4, p4_x, y_rf, 'Port 4: SMA RF Out', '+23.0 mm', '46.0 mm', '#ff2222', (p4_x - 100, y_rf - 280)),
]
...
ax.text((p1_x + p2_x)/2, y_rf + 75, 'INPUT CPWG TRACE\nL = 8.5 mm (Loss: 0.13 dB)', 
...
ax.text((p3_x + p4_x)/2, y_rf + 75, 'OUTPUT CPWG TRACE\nL = 20.5 mm (Loss: 0.29 dB)', 
        ha='center', va='center', color='#00f0ff', fontsize=11, fontweight='bold',
        bbox=dict(boxstyle='round,pad=0.4', fc='#0b1924', ec='#00f0ff', lw=1.5, alpha=0.85), zorder=8)

ax.text((p2_x + p3_x)/2, y_rf + 195, 'ACTIVE CIRCUIT & MATCHING ZONE\nVia Fence & Ground Isolation (~80 dB S41 / S31)', 
...
ax.text(w/2, 60, 'openEMS 3D FDTD Electromagnetic Port Locations on Physical PCB', 
...
legend_text = (
    ...
    '  * Port 1 (X = -23.0 mm): SMA RF Input end-launch interface (50 Ohm to generator)\n'
    '  * Port 2 (X = -14.5 mm): Input Match Pad (interface to discrete L1, C2, C1)\n'
    '  * Port 3 (X = +2.5 mm): Output Match Pad (interface to discrete C7 and collector tank)\n'
    '  * Port 4 (X = +23.0 mm): SMA RF Output end-launch interface (50 Ohm to receiver & bias-tee)\n'
